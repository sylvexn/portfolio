...
function ContactCard({ option }: { option: ContactOption }) {
  ...
  return (
    <motion.div variants={itemVariants} whileHover={{ scale: 1.02 }}>
      <Card
        className="p-6 hover:border-primary/30 transition-all duration-200 bg-card/50 cursor-pointer group"
        onClick={handleClick}
      >
        <div className="flex flex-col items-center text-center space-y-4">
          ...
          <div className="space-y-1">
            <h3 className="text-lg font-medium group-hover:text-primary transition-colors flex items-center justify-center gap-1">
              {option.label}
              {option.href && <ExternalLink className="h-3 w-3 opacity-70" />}
            </h3>
...
export function ContactModal({ isOpen, onClose }: ContactModalProps) {
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-[95%] sm:max-w-2xl max-h-[85vh] overflow-y-auto bg-background/95 backdrop-blur-md border-border/50">
        <DialogHeader>
